$(document).ready(function () {
  ...
  function filterPets() {
    $.get("/api/pets", function (data) {
      var petCards = [];
      // First filter is to check if user clicked lost, found, or all
  ...
  $("#lost").on("click", function () {
    ...
    // If petOrder has not option checked it comes back 'underfined'
    // If #petType has not been selected it come back 'null'
    // If there is no valule in #zipcode it comes back as an empty string
  ...
  $("#petType").on("mouseup", function () {
    ...
    //   document.querySelector('select[name="petType"]').onchange = changeEventHandler;
    // }, false);
